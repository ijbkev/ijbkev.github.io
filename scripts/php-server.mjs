...
export function startPhp({ preview = false } = {}) {
  ...
  const signature = path.join(storage, 'coordinator-signature.png');
  if (!existsSync(signature)) { copyFileSync('.local/signing/coordinator-signature.png', signature); chmodSync(signature, 0o600); }
