    ...
    <form key={formVersion} onSubmit={submit} className="space-y-8">
      <fieldset disabled={busy || accessExpired} className="space-y-8 min-w-0">
        ...
        <section className="rounded-2xl border border-violet-200/80 bg-violet-50/25 p-5 shadow-sm md:p-8 space-y-6 dark:border-violet-900 dark:bg-violet-950/10">
          ...
          <div className="rounded-xl border border-violet-200 bg-background p-4 space-y-3 shadow-sm dark:border-violet-900"><h3 className="text-sm font-semibold">Participant signature *</h3><p className="text-sm text-muted-foreground">To support signature security and verification, we collect your IP address (stored in masked form), signing time, and browser/device information when you submit. These details appear beside your signature in the travel reimbursement PDF and, if selected, the green travel declaration.</p><SignaturePad initialValue={signature} onChange={setSignature} disabled={busy || needUnlock} /></div>
