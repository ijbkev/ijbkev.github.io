  ...
  return <div className="pb-20">
    ...
    <div className="page-shell grid lg:grid-cols-[1.4fr_1fr] gap-10 py-12 items-start">
      ...
      <aside className="rounded-2xl border bg-card p-6 md:p-8 space-y-5 shadow-sm">
        ...
        {settings.data?.enabled ? <Button asChild className="w-full"><Link to={`/projects/${project.id}/reimbursement`}>Enter reimbursement portal<ArrowRight className="ml-2 w-4 h-4" /></Link></Button> : <p className="rounded-lg bg-muted p-3 text-sm">{settings.isError ? 'Reimbursement is temporarily unavailable.' : 'Reimbursement will open when the organizer has configured this project.'}</p>}
