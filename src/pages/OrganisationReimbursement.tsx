...
function OrganisationForm({ projectId, settings }: { projectId: string; settings: ProjectSettings }) {
  ...
  const profile = usePartnerProfile(projectId, country, saved => { setOrganisationName(saved.name); setOrganisationOid(saved.oid); });
  ...
  if (!profile.loaded) return <p role="status">{profile.status}</p>;
  ...
  return <form onSubmit={submit} className="space-y-7">
    <section className="rounded-2xl border border-sky-200 bg-sky-50/30 p-5 md:p-8 space-y-6">
      <div className="flex gap-3"><Landmark className="h-7 w-7 text-primary" /><div><h2 className="text-xl font-semibold">Organisation and country</h2><p className="text-sm text-muted-foreground">Your organisation name and OID are filled from Step 1. Your access code determines the country.</p></div></div>
