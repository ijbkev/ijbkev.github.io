...
export default function OrganisationReimbursement() {
  ...
  const project = projects.find(item => item.id === projectId && item.status === 'Upcoming');
...
function OrganisationForm({ projectId, settings }: { projectId: string; settings: ProjectSettings }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const parsed = organisationDeclarationSchema.safeParse({ requestId, organisationName, country, legalRepresentativeName, signaturePlace, signatureDate, accountHolder, iban, bankCountry, swift, signature, declaration });
