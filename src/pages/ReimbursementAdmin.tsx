...
import { claimReference, euro, reimbursement, extraSchema, settingsSchema, type ProjectSettings, type ClaimSummary, type SavedClaim, type OrganisationDeclarationSummary, type PartnershipAgreementSummary } from '../../shared/reimbursement';
...
function ProjectDashboard({ project }: { project: AdminProject }) {
  ...
  const [organisationAccessCode, setOrganisationAccessCode] = useState('');
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setAccessCode(''); setOrganisationAccessCode(''); setMessage('Project settings saved. Share each access code only with its intended users.');
...
<p className="text-xs text-muted-foreground">These countries appear on the project page and are the only options participants can select.</p>
          <fieldset disabled={busy}><legend className="font-semibold mb-3">Country reimbursement limits (EUR)</legend>
            <div className="grid sm:grid-cols-2 gap-3">{countryList.map((country, i) => <div key={country} className="rounded-lg bg-muted/40 p-3"><Field id={`country-limit-${i}`} label={`${country} — maximum (EUR)`} required type="number" min="0" max="100000000" step="0.01" placeholder="309.00" value={countryLimits[country] ?? ''} onChange={e => setCountryLimits(v => ({ ...v, [country]: e.target.value }))} /></div>)}</div>
          </fieldset>
        </div>
      </div>
<div className="rounded-xl border border-blue-200 bg-blue-50/40 p-4 space-y-3"><h3 className="font-semibold">Separate portal access codes</h3><div className="grid md:grid-cols-2 gap-4"><Field id="new-access-code" label={project.settings.hasAccessCode ? 'New participant access code (optional)' : 'Participant access code'} type="password" required={!project.settings.hasAccessCode} minLength={8} maxLength={128} autoComplete="new-password" value={accessCode} onChange={e => setAccessCode(e.target.value)} disabled={busy} /><Field id="new-organisation-access-code" label={project.settings.hasOrganisationAccessCode ? 'New partner organisation access code (optional)' : 'Partner organisation access code'} type="password" required={!project.settings.hasOrganisationAccessCode} minLength={8} maxLength={128} autoComplete="new-password" value={organisationAccessCode} onChange={e => setOrganisationAccessCode(e.target.value)} disabled={busy} /></div><p className="text-xs text-muted-foreground">Use different codes of at least 8 characters. Saved codes cannot be displayed. Changing either code signs out existing project sessions. Access codes do not appear in PDFs.</p></div>
