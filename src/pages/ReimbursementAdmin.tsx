...
import { claimReference, euro, reimbursement, extraSchema, settingsSchema, type ProjectSettings, type ClaimSummary, type SavedClaim, type OrganisationDeclarationSummary } from '../../shared/reimbursement';
  ...
  return <div className="space-y-8">
    <div className="grid lg:grid-cols-[1fr_1.3fr] gap-6 items-start">
      <form onSubmit={save} className="rounded-2xl border bg-card p-6 space-y-5">
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50/40 p-4 space-y-4"><h3 className="font-semibold">Separate portal access codes</h3><Field id="new-access-code" label={project.settings.hasAccessCode ? 'New participant access code (optional)' : 'Participant access code'} type="password" required={!project.settings.hasAccessCode} minLength={8} maxLength={128} autoComplete="new-password" value={accessCode} onChange={e => setAccessCode(e.target.value)} disabled={busy} /><Field id="new-organisation-access-code" label={project.settings.hasOrganisationAccessCode ? 'New partner organisation access code (optional)' : 'Partner organisation access code'} type="password" required={!project.settings.hasOrganisationAccessCode} minLength={8} maxLength={128} autoComplete="new-password" value={organisationAccessCode} onChange={e => setOrganisationAccessCode(e.target.value)} disabled={busy} /><p className="text-xs text-muted-foreground">Use different codes of at least 8 characters. Saved codes cannot be displayed. Changing either code signs out existing project sessions. Access codes do not appear in PDFs.</p></div>
