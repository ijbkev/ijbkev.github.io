...
scope.onmessage = async ({ data }) => {
  try {
    ...
    const result = await scope.PdfTicketForensics.analyzePdf(data.bytes, { fileName: data.filename, claimAmount: data.currency === 'EUR' ? data.amount : null, maxBytes: 10 * 1024 * 1024 });
