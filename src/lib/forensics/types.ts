...
export type PdfTicket = { serial: number; key?: string; label?: string; type?: string; isBoardingPass?: boolean; filename: string; from: string; to: string; amount: number; currency: string; url: string; error?: string };
export type TicketReport = PdfTicket & { result?: ScanResult; error?: string };
export type TicketManifest = { participant: string; tickets: PdfTicket[]; documents?: PdfTicket[]; skippedImages: number };
