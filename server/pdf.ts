...
import { claimReference, organisationName, reimbursement, legacyDeclarationText, type SavedClaim } from '../shared/reimbursement';

export type TicketFile = { bytes: Uint8Array; type: 'application/pdf' | 'image/png' | 'image/jpeg'; name: string };
...
export async function generatePdf(claim: SavedClaim, files: TicketFile[]) {
  ...
  function write(value: string, x: number, yy: number, size = 10, color = navy, target = page) {
    target.drawText(clean(value), { x, y: yy, size, font, color });
  }
  ...
  const attachmentLinks: { page: PDFPage; y: number; serial: number }[] = [];
  tableHeader();
  for (const ticket of claim.tickets) {
    const route = lines(`${ticket.from} > ${ticket.to}`, 165, font, 9);
    ...
    attachmentLinks.push({ page, y, serial: ticket.serial });
    y -= 15;
  ...
  if (signature.width > 2000 || signature.height > 1000) throw new Error('Signature image is too large');
  page.drawImage(signature, { x: 40, y: y - 75, ...signature.scaleToFit(240, 65) }); y -= 92;
  ...
  const ticketPages = new Map<number, PDFPage>();
  for (let i = 0; i < files.length; i++) {
    const ticket = claim.tickets[i];
    const file = files[i];
    ...
    if (file.type === 'application/pdf') {
      ...
      if (source.getPageCount() < 1 || source.getPageCount() > 10) throw new Error(`Ticket ${i + 1}: PDF must have 1 to 10 pages`);
      const form = source.getForm();
      if (form.getFields().length) form.flatten();
      for (const sourcePage of source.getPages()) {
        const box = sourcePage.getCropBox();
        if (!(box.width > 0 && box.height > 0) || box.width / box.height > 20 || box.height / box.width > 20) throw new Error(`Ticket ${i + 1}: unsupported page dimensions`);
      ...
      if (img.width * img.height > 25000000) throw new Error(`Ticket ${i + 1}: image exceeds 25 megapixels`);
    }
    const heights = pieces.map(p => p.height * Math.min(515 / p.width, 680 / p.height));
    const projectLines = lines(`${clean(claim.projectName)} | ${claim.projectCode} | ${claim.participant.name}`, 515, font, 9);
    const routeLines = lines(`${ticket.from} > ${ticket.to} | Travel: ${ticket.travelDate}`, 515, font, 10);
    const headerHeight = 99 + projectLines.length * 13 + routeLines.length * 14;
    const ticketHeight = Math.max(841.89, headerHeight + 60 + heights.reduce((a, b) => a + b, 0) + Math.max(0, pieces.length - 1) * 25);
    const ticketPage = doc.addPage([595.28, ticketHeight]);
    ticketPages.set(ticket.serial, ticketPage);
    ticketPage.drawRectangle({ x: 0, y: ticketHeight - 64, width: 595.28, height: 64, color: navy });
    write(`Ticket ${i + 1} of ${files.length}  |  ${ticket.mode}`, 40, ticketHeight - 40, 19, rgb(1, 1, 1), ticketPage);
  ...
  for (const link of attachmentLinks) {
    const destination = ticketPages.get(link.serial)!;
